...
interface IERC {
    ...
    function mint(address founders) external;
...
contract ERC20 is IERC {
    ...
    function mint(address founders) external virtual {
        _mint(founders, 1);
    }
...
contract FounderzNFT is ERC721, ERC721Enumerable, Ownable, ERC2981PerTokenRoyalties {
    ...
    using Strings for uint256;
    IERC public soulbound;

    Counters.Counter private _tokenIdTracker;

    uint256 private contractRoyalties = 1000;
    address public royaltiesReceiver = 0xBd4aCDBE00De933D25C632e1C777c246B1862417;
    string constant ContractCreator = "@FrankFourier";
    string public baseExtension = ".json";
    address[] public presaleWallets = [0xBd4aCDBE00De933D25C632e1C777c246B1862417];
    ...
    function isPresaleWallet(address _address) internal view returns (bool) {
        for (uint256 i = 0; i < presaleWallets.length; i++) {
            if (presaleWallets[i] == _address) {
                return true;
            }
        }
        return false;
    ...
    function buyPresale() external payable {
        require(isPresaleWallet(msg.sender), "Sender is not whitelisted");
        uint256 total = 10;
        require(_totalSupply() <= total, "Sale end");
